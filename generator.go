...
const (
	metaFileName string = "cmd/base.meta.go"
	blobFileName string = "service/base.meta.go"
	embedFolder  string = "resources/"
)

var conv = map[string]interface{}{"conv": fmtByteSlice}
var metaTmpl = template.Must(template.New("").Funcs(conv).Parse(`package cmd
...
func main() {
	...
	makeMetaFile(configs)
	makeBlobFile(configs)
}

func makeMetaFile(configs TemplateModel) {
	// Create meta file
	f, err := os.Create(metaFileName)
	if err != nil {
		log.Fatal("Error creating meta file:", err)
	}
	defer f.Close()
	...
	// Execute template
	if err = metaTmpl.Execute(builder, configs); err != nil {
	...
	// Writing meta file
	if err = ioutil.WriteFile(metaFileName, data, os.ModePerm); err != nil {
		log.Fatal("Error writing meta file", err)
	}
}
